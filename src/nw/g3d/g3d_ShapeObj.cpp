...
void Sphere::Merge(const Sphere &lhs, const Sphere &rhs) {
  ...
  if (distSq < epsilon) {
    *this = lhs.radius > rhs.radius ? lhs : rhs;
    ...
    this->center.Add(lhs.center,
                     diff.Mul(diff, (newRadius - lhs.radius) / dist));
...
#if defined(__ghs__)
  this->min.ps[0] = __PS_SEL(__PS_SUB(rhs.min.ps[0], lhs.min.ps[0]),
                             lhs.min.ps[0], rhs.min.ps[0]);
  this->max.ps[0] = __PS_SEL(__PS_SUB(lhs.max.ps[0], rhs.max.ps[0]),
                             lhs.max.ps[0], rhs.max.ps[0]);
#else
...
void ViewVolume::SetFrustum(float top, float bottom, float left, float right,
                            float zNear, float zFar, const Mtx34 &viewToWorld) {
...
void ShapeObj::Sizer::Calc(const InitArg &arg) {
  ...
  size_t subboundingSize =
      boundingEnabled
          ? Align(sizeof(AABB) * pRes->GetBoundingNodeCount(), CACHE_BLOCK_SIZE)
          : 0;
...
bool ShapeObj::Init(const InitArg &arg, void *pBuffer, size_t bufferSize) {
  ...
  NW_G3D_WARNING(IsAligned(pBuffer, BUFFER_ALIGNMENT),
                 "pBuffer must be aligned.");
...
bool ShapeObj::SetupBlockBuffer(void *pBuffer, size_t bufferSize) {
  ...
  for (int idxShpBlock = 0, numShpBlock = GetShpBlockCount();
    ...
    for (int idxBuffer = 0; idxBuffer < m_NumBuffering; ++idxBuffer) {
      ShpBlock *pShpBuffer =
          static_cast<ShpBlock *>(shpBlock.GetData(idxBuffer));

      if (IsBlockSwapEnabled()) {
        Copy32<true>(&pShpBuffer->worldMtx, &worldMtx, sizeof(Mtx34) >> 2);
        Copy32<true>(&pShpBuffer->vtxSkinCount, &vtxSkinCount,
                     sizeof(s32) >> 2);
      } else {
        Copy32<false>(&pShpBuffer->worldMtx, &worldMtx, sizeof(Mtx34) >> 2);
        Copy32<false>(&pShpBuffer->vtxSkinCount, &vtxSkinCount,
                      sizeof(s32) >> 2);
      }
...
void ShapeObj::CalcSubBounding(const SkeletonObj *pSkeleton) {
  ...
  const Mtx34 *pWorldMtxArray = pSkeleton->GetWorldMtxArray();
  const Mtx34 &worldMtx = pWorldMtxArray[GetBoneIndex()];
  const Bounding *pBoundingArray =
      m_pRes->ref().ofsSubBoundingArray.to_ptr<Bounding>();
#if NW_G3D_IS_HOST_CAFE
  size_t bufferSize =
      Align(sizeof(AABB) * m_pRes->GetBoundingNodeCount(), CACHE_BLOCK_SIZE);
...
void ShapeObj::CalcShpBlock(int viewIndex, const Mtx34 &worldMtx,
                            int bufferIndex /*= 0*/) {
...
bool ShapeObj::TestSubMeshIntersection(CullingContext *pCtx,
...

  {
    ...
    while (true) {
      ...
      int intersect =
          volume.TestIntersectionEx(m_pSubBounding[currentNodeIndex]);
      if (intersect > 0) {
        found = true;
      } else {
        if (intersect < 0) {
  ...
  if (idxOuterBound != idxInnerBound) {
    ...
    while (true) {
      ...
      if (found) {
        ...
        pCtx->submeshCount =
            pNode->numSubMesh + pNode->idxSubMesh - pCtx->submeshIndex;
      }
