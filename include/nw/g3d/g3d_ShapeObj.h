...
struct ViewVolume {
  ...
  void SetPerspectiveOffset(float fovy, float aspect, float zNear, float zFar,
                            const Mtx34 &viewToWorld, const Vec2 &offset);
  void SetFrustum(float top, float bottom, float left, float right, float zNear,
                  float zFar, const Mtx34 &viewToWorld);
  void SetOrtho(float top, float bottom, float left, float right, float zNear,
                float zFar, const Mtx34 &viewToWorld);
...
public:
  ...
  ShapeObj()
      : m_pRes(NULL), m_pUserPtr(NULL), m_pBufferPtr(NULL),
        m_pBlockBuffer(NULL) {}
  ...
  template <typename T> T *GetUserPtr() { return static_cast<T *>(m_pUserPtr); }

  template <typename T> const T *GetUserPtr() const {
  ...
  int GetVertexBufferCount() const {
    return GetResVertex()->GetVtxBufferCount();
  }
  ...
  const u16 *GetSubMeshIndexArray() const {
    return m_pRes->GetSubMeshIndexArray();
  }

  bool TestSubMeshIntersection(CullingContext *pCtx,
  ...
  NW_G3D_DEPRECATED_FUNCTION(bool IsBlendWeightValid() const) {
    return 0 != (m_Flag & BLEND_WEIGHT_VALID);
  }
...
public:
  explicit InitArg(ResShape *resource)
      : m_pRes(resource), m_NumBuffering(1), m_NumView(1),
        m_ViewDependent(false), m_BoundingEnabled(false)
